...
begin
  ...
  loop
    qual := array_append(qual, rec.team_id);
    qnames := array_append(qnames, rec.name);
  end loop;
  ...
  while coalesce(array_length(ord,1),0) < sz loop
    ...
    nord := '{}';
    for i in 1..len loop
      nord := array_append(nord, ord[i]);
      nord := array_append(nord, 2*len + 1 - ord[i]);
    end loop;
    ord := nord;
  end loop;
  ...
  for i in 1..sz loop
    if ord[i] <= n then
      cur_team  := array_append(cur_team, qual[ord[i]]);
      cur_label := array_append(cur_label, qnames[ord[i]]);
    else
      cur_team  := array_append(cur_team, null::uuid);
      cur_label := array_append(cur_label, 'Bye'::text);
    end if;
  ...
  while array_length(cur_team,1) > 1 loop
    ...
    for i in 1..mc loop
      ...
      if ma is null and mb is null and (ta is null) <> (tb is null) then
        nx_team  := array_append(nx_team, coalesce(ta, tb));
        nx_match := array_append(nx_match, null::uuid);
        nx_label := array_append(nx_label, case when ta is not null then cur_label[2*i-1] else cur_label[2*i] end);
        continue;
      end if;
      ...
      nx_team  := array_append(nx_team, null::uuid);
      nx_match := array_append(nx_match, new_id);
      nx_label := array_append(nx_label, ('Vinner ' || mlabel)::text);
    end loop;
